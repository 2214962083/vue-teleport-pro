...
const vm = {
  ...
  render() {
    ...
    const slotsDefaultVnode = typeof $slots.default === 'function' ? $slots.default() : $slots.default;
    ...
    const mountSlot = (element) => {
      const vueOptions = {
        ...
        render() {
          ...
          // return h('div', { id: uuid }, [
          //   slotDefault || null,
          // ]);
          return (
            <div id={uuid}>{slotDefault}</div>
          );
        },
      };
    ...
    setTimeout(() => {
      ...
      const uuidElement = targetElement && targetElement.querySelector(`#${uuid}`);
  ...
  methods: {
    ...
    destroySlotVm() {
      // 目标元素
      const targetElement = document.getElementById(this.to);

      // slotVm 实例内的 uuid element
      const uuidElement = targetElement && targetElement.querySelector(`#${this.uuid}`);
  ...
  beforeDestroy() {
    this.destroySlotVm();
    if (document.getElementById(this.to)) {
      document.body.removeChild(document.getElementById(this.to));
    }
  },
  // vue 3.x 销毁周期钩子
  beforeUnmount() {
